...
do
    vm_name="${klantnaam}-${servertype,,}-test$i" # Zet servertype naar lowercase
    vm_ip="192.168.2.$((ip_start+i))"
cat >> "${klantdir}/Vagrantfile" <<EOF
...
echo "De Vagrantfile en Ansible playbook voor $servertype zijn gegenereerd in de map $klantnaam voor $aantal_machines machines, elk met een uniek IP in de 192.168.2.x range."
...
echo "Uitrollen is voltooid. $aantal_machines machines voor $klantnaam ($servertype) met $vm_ram MB RAM zijn nu actief, elk met een uniek IP-adres."
